...
public class FRM_PhanCong extends JPanel {
	...
	private I_DAO_CongDoan congDoan_DAO;
	private DefaultTableModel modelDSTLD;
	private I_DAO_ThoLamDan thoLamDan_DAO;
	private DefaultTableModel modelDSPC;
	private I_DAO_BangPhanCong bangPhanCong_DAO;
	...
	public FRM_PhanCong() {
		...
		try {
			congDoan_DAO = (I_DAO_CongDoan) Naming.lookup(URL + "I_DAO_CongDoan");
			thoLamDan_DAO = (I_DAO_ThoLamDan) Naming.lookup(URL + "I_DAO_ThoLamDan");
			bangPhanCong_DAO = (I_DAO_BangPhanCong) Naming.lookup(URL + "I_DAO_BangPhanCong");
			...
			loadDataTLD(thoLamDan_DAO.getAlListThoLamDan());
			...
			loadDataPC(bangPhanCong_DAO.getBangPhanCongTheoNgayPhanCong(ngayPhanCong));
								...
								if (soLuongSanPham < 50) {
									...
									if (duplicate) {
										...
										Object[] rowData = new Object[] { bangPhanCong.getCongDoan().getMaCongDoan(),
												...
												bangPhanCong.getThoLamDan().getMaCongNhanVien(),
												bangPhanCong.getThoLamDan().getHoTen(),
	...
	private void loadDataCD(List<CongDoan> dsCD) {
	...
	private void loadDataTLD(List<ThoLamDan> dsTLD) {
	...
	private void loadDataPC(List<BangPhanCong> dsPC) {
		modelDSPC.setRowCount(0);
		for (entity.BangPhanCong bangPhanCong : dsPC) {
			Object[] objects = { bangPhanCong.getCongDoan().getMaCongDoan(),
					bangPhanCong.getCongDoan().getTenCongDoan(), bangPhanCong.getThoLamDan().getMaCongNhanVien(),
					bangPhanCong.getThoLamDan().getHoTen(), bangPhanCong.getCongDoan().getDan().getMaSanPham(),

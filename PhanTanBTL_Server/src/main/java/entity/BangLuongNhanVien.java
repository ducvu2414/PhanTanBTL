...
public class BangLuongNhanVien implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -8106750552415077483L;
	@Id
	@GeneratedValue(generator = "blnv_id_gen")
	@GenericGenerator(name = "blnv_id_gen", strategy = "util.BangLuongNhanVienIdGenerator")
	...
	public double tinhLuongThucTe(double luongCoBan, double heSoLuong, double phuCapThamNien) {
		double luongThucTe = luongCoBan + (soNgayThuongDiLam * luongCoBan * heSoLuong) / 26
				+ soGioTangCaNgayThuong * 25000 + soNgayLamChuNhat * 1.5 * (luongCoBan * heSoLuong) / 26
				+ soGioTangCaChuNhat * 25000 * 1.5;
		return luongThucTe;
	}
	...
	public double tinhLuongThucLinh(double luongThucTe, double luongCoBan, double heSoLuong) {
		double luongThucLinh = luongThucTe - (luongThucTe * 0.08 + luongThucTe * 0.015 + luongThucTe * 0.01)
				+ nhanVien.tinhPhuCapThamNienNhanVien(luongCoBan, nhanVien.tinhHeSoLuong());

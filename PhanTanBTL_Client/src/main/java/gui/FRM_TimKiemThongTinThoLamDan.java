...
public class FRM_TimKiemThongTinThoLamDan extends JPanel implements ActionListener {
	...
	private I_DAO_ThoLamDan dao_tld;
	...
	public FRM_TimKiemThongTinThoLamDan() {
		try {
			dao_tld = (I_DAO_ThoLamDan) Naming.lookup(URL + "I_DAO_ThoLamDan");
			setLayout(null);
			...
			String[] col = { "Mã Thợ Làm Đàn ", "Họ Tên Thợ Làm Đàn", "Giới Tính", "Ngày Sinh", "CMND", "SDT",
					"Địa Chỉ", "Tay Nghề", "Trạng Thái" };
	...
	public void updateTableDataThoLamDan() {
		...
		for (ThoLamDan thoLamDan : listtld) {
			...
			String[] rowData = { thoLamDan.getMaCongNhanVien(), thoLamDan.getHoTen(), gioiTinh,
					...
					thoLamDan.getSoDienThoai(), thoLamDan.getDiaChi(), thoLamDan.getTayNghe().getValue(),
					thoLamDan.isTrangThai() ? "Đang làm" : "Nghỉ làm" };

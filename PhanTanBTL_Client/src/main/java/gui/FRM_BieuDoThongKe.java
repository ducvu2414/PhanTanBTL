...
public class FRM_BieuDoThongKe extends JPanel {
	...
	private static I_DAO_ThongKe dao_ThongKe;
	...
	public FRM_BieuDoThongKe() {
		...
		setLayout(null);
		try {
			dao_ThongKe = (I_DAO_ThongKe) Naming.lookup(URL + "I_DAO_ThongKe");
		...
		btnTmKim.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				List<Object[]> results;
				try {
					results = dao_ThongKe.getTopLuongNhanVien(thang, nam);
					model.setRowCount(0);
					for (Object[] result : results) {
						Double luongThucTe = (Double) result[4];
	...
	public JFreeChart createBarChart(int thang, int nam) {
		...
		try {
			...
			for (Object[] result : results) {
				...
				dataset.addValue(luongThucTe, "Thu nhập", hoTen);
			}

			JFreeChart chart = ChartFactory.createBarChart("Top thợ làm đàn có thu nhập cao", "Họ tên",
					"Thu nhập (VNĐ)", dataset, PlotOrientation.VERTICAL, true, true, false);

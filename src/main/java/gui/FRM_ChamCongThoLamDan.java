...
public class FRM_ChamCongThoLamDan extends JPanel implements ActionListener {
	...
	private DAO_ChamCongThoLamDan dao_ChamCongThoLamDan;
	private DAO_ThoLamDan dao_ThoLamDan;
	private DAO_CongDoan dao_congDoan;
	private DAO_LuongThoLamDan dao_LuongThoLamDan;
	private DAO_Dan dao_dan;
	private DAO_BangPhanCong dao_BangPhanCong;
	...
	public FRM_ChamCongThoLamDan() {
		try {
			dao_ChamCongThoLamDan = new DAO_ChamCongThoLamDan();
			dao_ThoLamDan = new DAO_ThoLamDan();
			dao_congDoan = new DAO_CongDoan();
			dao_LuongThoLamDan = new DAO_LuongThoLamDan();
			dao_dan = new DAO_Dan();
			dao_BangPhanCong = new DAO_BangPhanCong();
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (output == JOptionPane.YES_OPTION) {
					try {
						...
						int check = 0;
						List<BangChamCongThoLamDan> listBCC = dao_ChamCongThoLamDan.layDanhSachChamCong(date);
						List<String> listtld = new ArrayList<>();
						for (BangChamCongThoLamDan bangChamCongThoLamDan : listBCC) {
							listtld.add(bangChamCongThoLamDan.getBangPhanCong().getThoLamDan().getMaCongNhanVien());
						}
						...
						for (int row = 0; row < model_BagPhanCong.getRowCount(); row++) {
							if (listtld.contains((String) model_BagPhanCong.getValueAt(row, 0))) {
								...
								if (!trangThaiDiLam.equals("Chưa ghi nhận chấm công")) {
									...
									BangPhanCong bangPhanCong = dao_BangPhanCong.getBangPhanCongTheoNgayTLD(date,
											maThoLamDan);
	...
	private void loadDataIntoTableChamCong() {
		...
		try {
			...
			List<BangPhanCong> listBangPhanCong = dao_ChamCongThoLamDan.listAllBangPhanCongTheoNgayHienTai();
			for (BangPhanCong bangPhanCong : listBangPhanCong) {
				if (listtld.contains(bangPhanCong.getThoLamDan().getMaCongNhanVien())) {
					Object[] objects = { bangPhanCong.getThoLamDan().getMaCongNhanVien(),
							bangPhanCong.getThoLamDan().getHoTen(), bangPhanCong.getCongDoan().getDan().getTenSanPham(),
	...
	private void loadDataIntoTableChamCongTheoTen(String ten) {
		...
		try {
			...
			if (sl == 0) {
				...
				List<BangPhanCong> listBangPhanCong = dao_ChamCongThoLamDan.listAllBangPhanCongTheoNgayHienTai();
				List<String> listmatld = new ArrayList<>();
				for (BangPhanCong bangPhanCong : listBangPhanCong) {
					listmatld.add(bangPhanCong.getThoLamDan().getMaCongNhanVien());
				}
				System.out.println("List ma tho lam dan:");
				System.out.println(listmatld);
				List<BangChamCongThoLamDan> listBCC = dao_ChamCongThoLamDan.layDanhSachChamCongTheoTen(date, ten);
				System.out.println("List BCC:");
				System.out.println(listBCC);
				ArrayList<String> listtld = new ArrayList<>();
				for (BangChamCongThoLamDan bangChamCongThoLamDan : listBCC) {
					listtld.add(bangChamCongThoLamDan.getBangPhanCong().getThoLamDan().getMaCongNhanVien());
				}
				List<ThoLamDan> listTLD = dao_ThoLamDan.getAlListThoLamDanTheoTen(ten);
				System.out.println("List TLD:");
				System.out.println(listTLD);
				for (ThoLamDan thoLamDan : dao_ThoLamDan.getAlListThoLamDanTheoTen(ten)) {
					if (listtld.contains(thoLamDan.getMaCongNhanVien())) {
						int position = listtld.indexOf(thoLamDan.getMaCongNhanVien());
						BangPhanCong bpc = dao_BangPhanCong.getBangPhanCongTheoNgayTLDSP(
								listBCC.get(position).getNgayChamCong(),
								listBCC.get(position).getBangPhanCong().getThoLamDan().getMaCongNhanVien(),
								listBCC.get(position).getBangPhanCong().getCongDoan().getMaCongDoan());
						Object[] objects = { listBCC.get(position).getBangPhanCong().getThoLamDan().getMaCongNhanVien(),
								listBCC.get(position).getBangPhanCong().getThoLamDan().getHoTen(),
								listBCC.get(position).getBangPhanCong().getCongDoan().getDan().getTenSanPham(),
								listBCC.get(position).getBangPhanCong().getCongDoan().getTenCongDoan(),
								bpc.getSoLuongSanPham(), listBCC.get(position).getSoLuongSanPham(), "Có mặt" };
						model_BagPhanCong.addRow(objects);
					} else {
						if (listmatld.contains(thoLamDan.getMaCongNhanVien())) {
							BangPhanCong bpc1 = dao_BangPhanCong.getBangPhanCongTheoNgayTLD(date,
									thoLamDan.getMaCongNhanVien());
							Object[] objects = { bpc1.getThoLamDan().getMaCongNhanVien(),
									bpc1.getThoLamDan().getHoTen(), bpc1.getCongDoan().getDan().getTenSanPham(),
									bpc1.getCongDoan().getTenCongDoan(), bpc1.getSoLuongSanPham(),
									bpc1.getSoLuongSanPham(), "Chưa ghi nhận chấm công" };
							model_BagPhanCong.addRow(objects);
						}
	...
	private void loadDataIntoTableChamCongTheoSanPham(String maSanPham) {
		...
		Date date = new Date(utilDate.getTime());
		try {
			List<BangChamCongThoLamDan> listBCC = dao_ChamCongThoLamDan.layDanhSachChamCongTheoSP(date, maSanPham);
			List<String> listtld = new ArrayList<>();
			for (BangChamCongThoLamDan bangChamCongThoLamDan : listBCC) {
				listtld.add(bangChamCongThoLamDan.getBangPhanCong().getThoLamDan().getMaCongNhanVien());
			}

			List<BangPhanCong> listBangPhanCong = dao_ChamCongThoLamDan
					.listAllBangPhanCongTheoNgayHienTaiVaSP(maSanPham);

			for (BangPhanCong bangPhanCong : listBangPhanCong) {
				if (listtld.contains(bangPhanCong.getThoLamDan().getMaCongNhanVien())) {
					int position = listtld.indexOf(bangPhanCong.getThoLamDan().getMaCongNhanVien());
					BangPhanCong bpc = dao_BangPhanCong.getBangPhanCongTheoNgayTLDSP(
							listBCC.get(position).getNgayChamCong(),
							listBCC.get(position).getBangPhanCong().getThoLamDan().getMaCongNhanVien(),
							listBCC.get(position).getBangPhanCong().getCongDoan().getMaCongDoan());
	...
	private void loadDataIntoTableChamCongTheoTenCongDoan(String tenCongDoan) {
		...
		Date date = new Date(utilDate.getTime());
		try {
			List<BangChamCongThoLamDan> listBCC = dao_ChamCongThoLamDan.layDanhSachChamCongTheoTenCongDoan(date,
					tenCongDoan);
			List<String> listtld = new ArrayList<>();
			for (BangChamCongThoLamDan bangChamCongThoLamDan : listBCC) {
				listtld.add(bangChamCongThoLamDan.getBangPhanCong().getThoLamDan().getMaCongNhanVien());
			...
			for (BangPhanCong bangPhanCong : listBangPhanCong) {
				if (listtld.contains(bangPhanCong.getThoLamDan().getMaCongNhanVien())) {
					...
					BangPhanCong bpc = dao_BangPhanCong.getBangPhanCongTheoNgayTLDSP(
							listBCC.get(position).getNgayChamCong(),
							listBCC.get(position).getBangLuongThoLamDan().getThoLamDan().getMaCongNhanVien(),
							listBCC.get(position).getBangPhanCong().getCongDoan().getMaCongDoan());
					System.out.println(listBCC.get(position).getBangPhanCong().getCongDoan().getMaCongDoan());
					Object[] objects = { listBCC.get(position).getBangPhanCong().getThoLamDan().getMaCongNhanVien(),
							listBCC.get(position).getBangPhanCong().getThoLamDan().getHoTen(),
							listBCC.get(position).getBangPhanCong().getCongDoan().getDan().getTenSanPham(),
							listBCC.get(position).getBangPhanCong().getCongDoan().getTenCongDoan(),
	...
	private void themBangLuong(ThoLamDan thoLamDan) {
		...
		try {
			...
			String maBangLuong = dao_LuongThoLamDan.getMaBangLuong(thang, nam, thoLamDan.getMaCongNhanVien());

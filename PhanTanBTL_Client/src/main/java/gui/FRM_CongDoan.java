...
public class FRM_CongDoan extends JPanel {
	...
	public FRM_CongDoan() {
		try {
			dan_DAO = (I_DAO_Dan) Naming.lookup(URL + "I_DAO_Dan");
...
//				txtMaCongDoan.setFont(new Font("Tahoma", Font.PLAIN, 16));
			...
			String[] cols = { "M\u00E3 s\u1EA3n ph\u1EA9m", "T\u00EAn s\u1EA3n ph\u1EA9m",
					...
					if (btnThem.getText().equals("Thêm")) {
						...
						if (row >= 0) {
							...
							if (kt) {
								...
								try {
									if (!congDoan_DAO.checkIfExists(tenCongDoan, maSanPham) == true) {
										...
										if (congDoan_DAO.insertCongDoan(congDoan)) {
										...
										tblDSCongDoan.clearSelection();
										tblDSSanPham.clearSelection();
										JOptionPane.showMessageDialog(null, "Công đoạn này đã tồn tại cho sản phẩm.",
						...
						if (rowCD >= 0) {
							...
							if (kt) {
								...
								Dan dan;
								try {
									dan = dan_DAO.getDanTheoMaSanPham(maSanPham);
									congDoan.setDan(dan);
									congDoan.setTenCongDoan((String) comboBox.getSelectedItem());
									congDoan.setGiaCongDoan(Float.parseFloat(txtGiaCongDoan.getText()));
									...
									loadDataCD(congDoan_DAO.getAlListCongDoan());
									modelDSCongDoan.fireTableDataChanged();
									for (CongDoan congDoan1 : congDoan_DAO.getAlListCongDoan()) {
	...
	private void loadDataSP(List<Dan> dsSanPham) {

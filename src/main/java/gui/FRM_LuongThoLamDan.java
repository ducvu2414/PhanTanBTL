...
public class FRM_LuongThoLamDan extends JPanel {
	...
	private static int rowSelectedOld = -1;
			...
			tbl_BangLuong.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					...
					if (row != rowSelectedOld) {
						...
						String maBangLuong;
						try {
							maBangLuong = dao_LuongThoLamDan.getMaBangLuong(thang, nam,
	...
	private void loadDataLuong() throws RemoteException {
		...
		for (ThoLamDan thoLamDan : dao_ThoLamDan.getAlListThoLamDan()) {
			...
			bl.setNam(nam);
			bl.setThang(thang);
			slSanPham = dao_LuongThoLamDan.laySoSanPham(thoLamDan.getMaCongNhanVien(), thang, nam);
			bl.setSoLuongSanPham(slSanPham);
			boolean KT = dao_LuongThoLamDan.kiemTraTrungMa(thang, nam, thoLamDan.getMaCongNhanVien());
			if (!KT) {
			...
			double luong = dao_LuongThoLamDan.layTongThuNhapTungThang(thoLamDan.getMaCongNhanVien(), thang, nam);
			Object[] objects = { thoLamDan.getMaCongNhanVien(), thoLamDan.getHoTen(),
					decimalFormat.format(thoLamDan.tinhPhuCapThamNienThoLamDan(thoLamDan.tinhHeSoLuong())),
					decimalFormat.format(luong), bl.tinhLuongThucLinh(luong) };
	...
	private void themBangLuong(ThoLamDan thoLamDan) throws RemoteException {
		int nam = yearChooser.getYear();
		int thang = Integer.parseInt(cmbThang.getSelectedItem().toString());
		BangLuongThoLamDan bl = new BangLuongThoLamDan();
		bl.setNam(nam);
	...
	private void loadDataLuongTheoTen(String tenThoLamDan) {
		...
		try {
			...
			if (sl == 0) {
				...
				int nam = yearChooser.getYear();
				int thang = Integer.parseInt(cmbThang.getSelectedItem().toString());
				List<BangLuongThoLamDan> listBL = dao_LuongThoLamDan.getBangLuongTheoTen(tenThoLamDan, thang, nam);
				for (BangLuongThoLamDan bangLuongThoLamDan : listBL) {
					...
					Object[] objects = { bangLuongThoLamDan.getThoLamDan().getMaCongNhanVien(),
